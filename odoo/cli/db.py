...
class Db(Command):
    ...
    def run(self, cmdargs):
        ...
        parser.add_option(
            "--restore_image", help="Image name from ERPLibre/image_db"
        )
        parser.add_option(
            "--master_password",
            help=(
                "Specify the master password if need it. Prefer the"
                " MASTER_PWD environment variable: an argument is visible"
        ...
        parser.add_option(
            "--demo",
            action="store_true",
            help="Add demo data in create database.",
        )
        parser.add_option(
            "--user_lang", default="fr_CA", help="Language like en_US."
        )
        parser.add_option(
            "--user_password",
            default="admin",
            help="User password in create database.",
        )
        parser.add_option(
            "--user_login",
            default="admin",
            help="User login in create database.",
        )
        parser.add_option(
            "--user_phone", help="User phone in create database."
        )
        parser.add_option(
            "--user_country_code",
            default="ca",
            help="Country code, search country_utils.py.",
        )
        ...
        parser.add_option(
            "--backup",
            action="store_true",
            help="Command backup database to export in .zip file. "
            "Need argument --restore_image or --restore_db_file.",
        )
        parser.add_option(
            "--drop", action="store_true", help="Command drop database."
        )
        parser.add_option(
            "--neutralize",
            action="store_true",
        ...
        parser.add_option(
            "--clone",
            action="store_true",
            help="Command clone database. " "Need argument --db_name_from",
        )
        parser.add_option(
            "--restore",
            action="store_true",
            help="Command restore database. "
            "Need argument --restore_image or --restore_db_file.",
        )
        parser.add_option(
            "--list", action="store_true", help="Command list database."
        )
        parser.add_option(
            "--list_incompatible_db",
            action="store_true",
            help="Command list database incompatible.",
        )
        parser.add_option(
            "--version", action="store_true", help="Command show odoo version."
        )
        ...
        die(
            bool(opt.restore)
            and not (bool(opt.restore_db_file) or bool(opt.restore_image)),
            "Missing argument --restore_db_file or --restore_image of option --restore.",
        )

        die(
            bool(opt.restore) and not bool(opt.db_name),
        ...
        die(
            bool(opt.backup) and not bool(opt.db_name),
            "Missing argument --database of option --backup.",
        )

        die(
            bool(opt.create) and not bool(opt.db_name),
            "Missing argument --database of option --create.",
        )

        die(
            bool(opt.clone) and not bool(opt.db_name),
            "Missing argument --database of option --clone.",
        )

        die(
            bool(opt.clone) and not bool(opt.db_name_from),
            "Missing argument --from_database of option --clone.",
        )

        die(
            bool(opt.drop) and not bool(opt.db_name),
        ...
        if opt.list:
            ...
            dispatch_rpc(
                "db", "drop", [_master_password(opt), opt.db_name]
            )
        elif opt.create:
            ...
            if opt.restore_image:
                file_name = (
                    opt.restore_image
                    if opt.restore_image.endswith(".zip")
                    else f"{opt.restore_image}.zip"
                )
            ...
            db.restore_db(
                opt.db_name,
                file_path,
                copy=not opt.move_database,
                neutralize_database=opt.neutralize,
            )
        elif opt.clone:
