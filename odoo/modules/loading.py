...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if tools.config.get('uninstall'):
            ...
            if uninstall_ids:
                update_module = True
                cr.execute("update ir_module_module set state=%s where id in %s and state=%s",
                           ('to remove', (tuple(uninstall_ids.ids)), 'installed'))
                _logger.info('Uninstalling modules (including all dependencies installed) %s',
                             uninstall_ids.mapped('name'))
            else:
